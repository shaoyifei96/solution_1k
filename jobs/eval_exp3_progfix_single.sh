...
#SBATCH --mem=250G
#SBATCH --output=/vast/projects/kumar/lab/yishao/jobs/logs/eval_pf-%j.out
#SBATCH --error=/vast/projects/kumar/lab/yishao/jobs/logs/eval_pf-%j.err
#SBATCH --exclude=dgx021

# Usage: sbatch eval_exp3_progfix_single.sh <TASK_NAME> <INSTANCE_ID>
TASK_NAME=$1
INSTANCE_ID=$2
...
LOG_PATH=$HOME_DIR/eval_logs/${TASK_NAME}_exp3_progfix_${SLURM_JOB_ID}

mkdir -p $HOME_DIR/eval_logs $HOME_DIR/jobs/logs
module load anaconda3; source "$(conda info --base)/etc/profile.d/conda.sh"

hostname
echo "=== Eval exp3 PROGRESS-FIX: $TASK_NAME instance $INSTANCE_ID ==="
echo "Start: $(date)"
