...
#SBATCH --mem=250G
#SBATCH --output=/vast/projects/kumar/lab/yishao/jobs/logs/eval_e2ph-%j.out
#SBATCH --error=/vast/projects/kumar/lab/yishao/jobs/logs/eval_e2ph-%j.err
#SBATCH --exclude=dgx021

# Usage: sbatch eval_exp2_progresshead_single.sh <TASK_NAME> <INSTANCE_ID>
TASK_NAME=$1
INSTANCE_ID=$2
...
LOG_PATH=$HOME_DIR/eval_logs/${TASK_NAME}_exp2_progresshead_${SLURM_JOB_ID}

mkdir -p $HOME_DIR/eval_logs $HOME_DIR/jobs/logs
module load anaconda3; source "$(conda info --base)/etc/profile.d/conda.sh"

hostname
echo "=== Eval exp2 PROGRESS-HEAD: $TASK_NAME instance $INSTANCE_ID ==="
echo "Start: $(date)"
...
uv run scripts/serve_b1k.py --port $PORT \
    ...
    policy:checkpoint --policy.config pi_behavior_b1k_exp2 --policy.dir $CHECKPOINT_DIR &
SERVER_PID=$!; sleep 45
